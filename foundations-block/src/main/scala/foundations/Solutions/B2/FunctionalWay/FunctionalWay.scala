...
object FunctionalWay extends App {
  ...
  def flatMapOption(maybeInt: Option[Int], fun: Int => Option[Int]): Option[Int] = maybeInt match {
  ...
  def listSum2RandomElements(list: List[Int], f: (Int, Int) => Int): Option[Int] = list match {
    case _                      => None
  ...
  def getProfit(stock: List[Clothes]): (Double, Double) = {
    ...
    val calTotalPriceProfit: PriceQuantityProfit => Double = (priceQuantityProfit: PriceQuantityProfit) =>
      priceQuantityProfit._1 * priceQuantityProfit._2 * priceQuantityProfit._3
    ...
    val totalPrices: List[(Double, Double)] =
      stock.map(cl => (getTotalPriceWithOutProfit(cl), getTotalPriceWithProfit(cl)))
  ...
  def getProfitTuple(stock: List[Clothes]): (Double, Double) = {
    ...
    val calTotalPriceProfit: (Double, Int, Double) => Double = (price: Double, quantity: Int, profit: Double) =>
