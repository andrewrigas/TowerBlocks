...
object FunctionalWay extends App {
  ...
  def flatMapOption(maybeInt: Option[Int], fun: Int => Option[Int]): Option[Int] = ???
  ...
  def listSum2RandomElements(list: List[Int], f: (Int, Int) => Int): Option[Int] = ???
  ...
  def applyDiscount(itemsList: List[Item], discount: Double): Double = {
    ...
    // andThen solution
    val _ = itemsList.map(item => (getPrice andThen applyDiscount)(item)).sum
